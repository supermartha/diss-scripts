...
function sendResults(){
  document.getElementById('main').innerHTML = "<p>...saving responses...</p>"
    ...
    type: 'POST',
    success: function(){location.href="SVP_part3_instructions.php"}} // Redirect to part 2 when finished
    );
  }
  ...
  function record_responses(keystroke) {
    ...
    document.cookie = "trap_answers=" + JSON.stringify(trap_answers);
    document.cookie = "bath_answers=" + JSON.stringify(bath_answers);
    document.cookie = "filler_word=" + JSON.stringify(filler_word);
    document.cookie = "filler_nonce=" + JSON.stringify(filler_nonce);
    //console.log(results)
    changeText();
  }
